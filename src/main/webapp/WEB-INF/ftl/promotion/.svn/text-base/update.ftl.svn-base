...
<br/>
			<form method="post" id="PromotionAddForm" action="<@spring.url "/promotions/edit/${promotion.id?c}"/>">
				<fieldset>
					<@spring.formHiddenInput "promotion.id"/>
					...
					<div class="input text">
						...
						<@spring.formInput "promotion.epromo", 'id="PromotionEpromo" maxlength="45"'/>
					</div>
					<div class="row date">
						<div class="span6">
					...
					<div class="row">
						<div class="span8">
							...
							<@spring.formSingleSelect "promotion.category", categoryOptions, 'class="span6 categorySelect"'/>
						</div>
						<div class="span8">
...
<script type="text/javascript">    
    $(document).ready(function() {
            ...
            $(document).on('change','#category',function(){                
                promotionsProducts("${promotion.id}", this.value);
            });
          <#if promotion.category??>
          $(".categorySelect option[value=${promotion.category.id}]").attr('selected', 'selected');
          promotionsProducts("${promotion.id}", "${promotion.category.id}");
          </#if>
          $('#PromotionAddForm').find("input[type=text]:first").focus();
    });
        
        var promotionsProducts = function(id, categoryId){
        	if(id.length > 0){
	        	$.get('<@spring.url "/promotions/products/"/>' + id + "/" + categoryId + "?ajax=true",function(data){
